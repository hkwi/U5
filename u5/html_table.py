...
#		print(x,y,col,row)
	...
	def __repr__(self):
		return repr((self.x, self.y, self.colspan, self.rowspan, dump_element(self.element)))

class Table(object):
	def __init__(self, et, quirks=False, span=False):
		self.quirks = quirks
		self.xw = self.yh = 0
		self.x = self.y = 0
		self.coords = {}
	...
	def matrix(self, conv=None):
		...
		for y in range(self.yh):
			...
			for x in range(self.xw):
				k = (x,y)
				cells = self.coords.get(k, [])
				assert len(cells) < 2, repr([k, self.yh, self.y])
				el = None
				if cells:
					cell = cells[0]
					if self.span or (x==cell.x and y==cell.y):
						el = cell.element
	...
	def proc_row(self, c):
		...
		for cc in cells:
			while self.x < self.xw and self.coords.get((self.x, self.y)):
				self.x += 1
